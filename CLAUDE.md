...
`Ctrl/Command+Shift+A` summons the avatar globally and `Ctrl/Command+Shift+V` is the fallback. One press or repeated presses must leave it visible and must never close it. Right-click provides Small, Medium, Large, Close, and Quit. Close leaves the shortcuts running. Size requests map to `size -Size small|medium|large`; 440x300 is Large and is the maximum.
