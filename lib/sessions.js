...
function parsePosixClaudeProcesses(raw) {
  return String(raw || "")
    .split(/\r?\n/)
    .map((line) => {
      const match = line.trim().match(/^(\d+)\s+(.+)$/);
      if (!match) return null;
      const session = match[2].match(SESSION_ID);
      if (!session || !/\bclaude(?:\s|$)/i.test(match[2])) return null;
      return {
        id: session[1].toLowerCase(),
        pid: Number(match[1]),
        createdAt: null
      };
    })
    ...
    return new Promise((resolve, reject) => {
      execFile(
        ...
        (error, stdout) => {
          ...
          resolve(parsePosixClaudeProcesses(stdout));
        }
      );
    });
  }
